    ...
    methods: {
      onSubmit: function() {
        ...
        if (this.english !== this.currWord.english) {
          isCorrect = false;
          this.flash('English wrong!', 'error', { timeout: 1000 });
          this.incorrectGuesses.push(this.currWord.german + ' (EN)');
        }
        if (this.french !== this.currWord.french) {
          isCorrect = false;
          this.flash('French wrong!', 'error', { timeout: 1000 });
          this.incorrectGuesses.push(this.currWord.german + ' (FR)');
        }
